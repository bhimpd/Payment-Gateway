...
export class Helper {
    ...
    async fillSearchParameterNClick(expectedSelector: Locator, expectedText: any) {
        ...
        await expect(expectedSelector).toHaveValue(expectedText);
    ...
    async assertNumericInputValue(expectedSelector: Locator, expectedValue: any) {
        const actualValue = await expectedSelector.inputValue();

        console.log("Actual Amount ==> ", actualValue);
        console.log("Expected Amount ==> ", expectedValue);
        expect(Number(actualValue)).toBeCloseTo(Number(expectedValue), 2);
    }
